...
function draw() {
  ...
  for (let swordsShown = 0; swordsShown < swords.length; swordsShown++) {
    let thisSword = swords[swordsShown];
    thisSword.move();
    thisSword.show();
    for (let cryptidsLeft = 0; cryptidsLeft < cryptids.length; cryptidsLeft++) {
      let proposedCatch = cryptids[cryptidsLeft];
      if (isTouching(thisSword, proposedCatch)) {
        //print ("slain");
        cryptids.splice(cryptidsLeft, 1);
        break;
      }
    }
  }
